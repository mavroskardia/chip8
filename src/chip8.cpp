...
Chip8::Chip8()
{
	...
	convolver = new Convolver(3, matrix);
}
...
bool Chip8::load(string filename)
{
	...
	if (file.is_open()) {
		...
		return program_length == program.size();
	}
...
string Chip8::translate_opcode(word oc)
{
	...
	case 0x2000:
		...
		break;
	case 0x3000:
		ss << "SE   V"  << hex << vx << " 0x" << hex << nn;
		break;
	case 0x4000:
		ss << "SNE  V" << hex << vx << " 0x" << hex << nn;
		break;
	case 0x5000:
		ss << "SE   V" << hex << vx << " V" << vy;
		break;
	case 0x6000:
		ss << "LD   V" << hex << vx << " 0x" << hex << nn;
		break;
	case 0x7000:
		ss << "ADD  V" << hex << vx << " 0x" << hex << nn;
		break;
	case 0x9000:
		ss << "SNE  V" << hex << vx << " V" << vy;
		break;
	case 0xa000:
		ss << "LD   I  0x" << hex << addr;
		break;
	case 0xb000:
		ss << "JP   V0 0x" << hex << addr;
		break;
	case 0xc000:
		ss << "RND  V" << hex << vx << ' ' << hex << nn;
		break;
	case 0xd000:
		ss << "DRW  V" << hex << vx << " V" << vy << " " << n;
		break;
	case 0x0000: {
		switch (oc & 0x00ff) {
		case 0xe0:
	...
	case 0x8000: {
		...
		case 0x1:
			...
			break;
		case 0x2:
			ss << "AND  V" << hex << vx << " V" << vy;
			break;
		case 0x3:
			ss << "XOR  V" << hex << vx << " V" << vy;
			break;
		case 0x4:
			ss << "ADD  V" << hex << vx << " V" << vy;
			break;
		case 0x5:
			ss << "SUB  V" << hex << vx << " V" << vy;
			break;
		case 0x6:
			ss << "SHR  V" << hex << vx;
			break;
		case 0x7:
			ss << "SUBN V" << hex << vx << " V" << vy;
			break;
		case 0xe:
			ss << "SHL  V" << hex << vx;
			break;
...
void Chip8::render()
{
	...
	vector<SDL_Rect> c8pixels;

	for (int y = 0; y < NumPixelsTall; y++) {
		for (int x = 0; x < NumPixelsWide; x++) {
			if (gfx[x + (NumPixelsWide * y)])
				c8pixels.push_back( { x * PixelWidth, y * PixelHeight, PixelWidth, PixelHeight });
		}
	}

	if (show_debug) render_debug();

	postprocess(&c8pixels);
...
void Chip8::postprocess(vector<SDL_Rect>* c8pixels)
{
	SDL_FillRects(surface, c8pixels->data(), c8pixels->size(), SDL_MapRGB(pixel_format, 100, 0, 0));

// TODO: Make convolver run fast enough to do this in realtime
//		convolver->run(surface, tmpsurface);
//		SDL_BlitSurface(tmpsurface, nullptr, surface, nullptr);
...
void Chip8::render_debug()
{
	...
	for (auto s : dbgout) {
		auto debug_surface = TTF_RenderUTF8_Blended(font, s.c_str(), {200,200,200});
		auto debug_texture = SDL_CreateTextureFromSurface(renderer, debug_surface);
		SDL_Rect r = {GameWidth + 50, y, debug_surface->w, debug_surface->h};
		y += debug_surface->h + 5;
		SDL_RenderCopy(renderer, debug_texture, NULL, &r);
...
int Chip8::initialize_screen()
{
	...
	surface = SDL_CreateRGBSurface(0, GameWidth, GameHeight, 32, 0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff);
	tmpsurface = SDL_CreateRGBSurface(0, GameWidth, GameHeight, 32, 0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff);
	texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC, GameWidth, GameHeight);
...
{
	SDL_FillRect(surface, nullptr, SDL_MapRGB(pixel_format, 0, 0, 0));
	redraw = true;
}
